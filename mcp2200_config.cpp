...
void print_help(void)
{
    ...
    cout << " --vid <unsigned int>\tSet usb vendor id\n";
    cout << " --pid <unsigned int>\tSet usb product id\n";
    cout << " --setVendorProductId \t Set vid / pid based on string passed in.\n";
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        static struct option long_options[] =
            {
                {"baud", required_argument, 0, 0},         //  0
                {"rx_led", required_argument, 0, 0},       //  1
                {"tx_led", required_argument, 0, 0},       //  2
                {"cts_rts", required_argument, 0, 0},      //  3
                {"usbcfg", required_argument, 0, 0},       //  4
                {"suspend", required_argument, 0, 0},      //  5
                {"invert_pol", required_argument, 0, 0},   //  6
                {"manufacturer", required_argument, 0, 0}, //  7
                {"product", required_argument, 0, 0},      //  8
                {"io_dir", required_argument, 0, 0},       //  9
                {"io_val", required_argument, 0, 0},       // 10
                {"vid", required_argument, 0, 0},          // 11
                {"pid", required_argument, 0, 0},          // 12

                {"show", no_argument, 0, 's'},
                {"vendorId", required_argument, 0, 'v'},
                {"productId", required_argument, 0, 'p'},
                {"setVendorProductId", no_argument, 0, 'x'},
        ...
        case 0:
            ...
            case 10:
                ...
                break;
            case 11:
                setVid = strtol(optarg, NULL, 0);
                break;
            case 12:
                setPid = strtol(optarg, NULL, 0);
                break;
            }
            break;
        case 'h':
    ...
    if (SimpleIOClass::IsConnected())
    {
        ...
        if(setVidPidFlag && (setPid > 0) && (setVid > 0))
        {
            cout << "Set VID to: 0x" << hex << setVid << dec << endl;
            cout << "Set PID to: 0x" << hex << setPid << dec << endl;
            SimpleIOClass::SetVendorIDProductID(setVid, setPid);
        }
